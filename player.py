...
class Player:
    ...
    def draw(self, screen):
        ...
        pygame.draw.circle(screen, BLACK, (self.x + 10, self.y - 5), 4)
        pygame.draw.arc(
            screen, BLACK, (self.x - 15, self.y, 30, 20), 0, 3.14, 2
        )
    ...
    def draw_bow(self, screen):
        ...
        WHITE = (255, 255, 255)
        pygame.draw.arc(
            screen, BROWN, (self.x + 10, self.y - 20, 30, 40), 0.5, 2.6, 4
        )
        pygame.draw.line(
            screen, WHITE, (self.x + 10, self.y), (self.x + 40, self.y), 2
        )
        ...
        if self.weapon_type == "axe":
            ...
            if self.attack_animation < 20:
                ...
                wave_surface = pygame.Surface(
                    (wave_radius * 2, wave_radius * 2), pygame.SRCALPHA
                )
                pygame.draw.circle(
                    wave_surface,
                    (255, 255, 0, alpha),
                ...
                screen.blit(
                    wave_surface, (self.x - wave_radius, self.y - wave_radius)
                )
        ...
        while i < len(self.projectiles):
            ...
            for enemy in enemies:
                ...
                if distance < enemy.radius + 5:
                    ...
                    lifesteal_amount = self.damage * (self.lifesteal / 100)
                    self.health = min(
                        self.max_health, self.health + lifesteal_amount
                    )
            ...
            if (
                collision_occurred
                or projectile["distance"] >= projectile["max_distance"]
            ):
    ...
    def draw_health_bar(self, screen):
        ...
        pygame.draw.rect(
            screen, GREEN, (20, 20, 200 * (self.health / self.max_health), 20)
        )
        pygame.draw.rect(screen, (100, 100, 100), (20, 45, 200, 10))
        pygame.draw.rect(
            screen,
            PURPLE,
            (20, 45, 200 * (self.experience / self.experience_to_level), 10),
        )

        font = pygame.font.SysFont(None, 24)
        level_text = font.render(
            f"Ур. {self.level} | {self.get_weapon_name()}", True, WHITE
        )
