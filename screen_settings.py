...
def draw_ui(screen, player, enemies, width, height, game_manager):
    ...
    difficulty_text = small_font.render(
        f"Сложность: {difficulty_info['level']}", True, YELLOW
    )
    screen.blit(difficulty_text, (width - 150, 80))

    # Множитель опыта
    exp_mult_text = small_font.render(
        f"Опыт: x{difficulty_info['exp_multiplier']:.1f}", True, CYAN
    )
    ...
    controls_text = small_font.render(
        "WASD: движение, SPACE: атака, ESC: меню", True, WHITE
    )
    ...
    pygame.draw.rect(
        screen, YELLOW, (20, height - 60, 200 * next_level_progress, 10)
    )
...
def draw_game_over(screen, width, height, player, game_manager):
    ...
    screen.blit(
        stats_text, (width // 2 - stats_text.get_width() // 2, height // 2 - 30)
    )
    screen.blit(
        restart_text,
    ...
    screen.blit(
        difficulty_text,
        (width // 2 - difficulty_text.get_width() // 2, height // 2 + 60),
    )
...
def draw_level_up_screen(screen, width, height, player):
    ...
    title_text = font.render(
        f"ПОЗДРАВЛЯЕМ! Уровень {player.level}", True, YELLOW
    )
    ...
    options = [
        {
            "type": "damage",
            "text": f"+10 урона (сейчас: {player.damage})",
            "key": "1",
        },
        {
            "type": "range",
            "text": f"+20 радиуса атаки (сейчас: {player.attack_range})",
            "key": "2",
        },
        {
            "type": "lifesteal",
            "text": f"+15% вампиризма (сейчас: {player.lifesteal}%)",
            "key": "3",
        },
    ]
    ...
    for i, option in enumerate(options):
        ...
        screen.blit(
            option_text, (width // 2 - option_text.get_width() // 2, y_pos + 15)
        )

        # Клавиша выбора
        key_text = small_font.render(f"Нажмите {option['key']}", True, GREEN)
        screen.blit(
            key_text, (width // 2 - key_text.get_width() // 2, y_pos + 40)
        )
